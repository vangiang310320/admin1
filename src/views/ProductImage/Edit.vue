  ...
  <v-dialog v-model="dialog" width="640px">
    <v-card flat tile>
      <v-card-title class="primary">
        <span style="color:white;">Sửa ảnh Sản phẩm</span>
      </v-card-title>
      <v-card-text>
        <v-form>
          <v-container>
      ...
      <v-card-actions>
        ...
        <v-btn color="grey darken-2" class="mr-2" dark @click="$emit('close')"
          >Huỷ</v-btn
        >
        <v-btn color="primary" class="mr-2" dark @click="updateProductImage"
          >Cập nhật</v-btn
        >
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    updateProductImage() {
      axios
        .put("https://localhost:44306/api/ProductImage/UpdateProductImage", this.data)
        .then(res => {
  ...
  watch: {
    currentItem: function() {
      this.data.imagePath = this.currentItem.imagePath;
      this.data.isDefault = this.currentItem.isDefault;
