  ...
  <v-dialog v-model="dialog" width="640px">
    <v-card flat tile>
      <v-card-title class="primary">
        <span style="color:white;">Thêm mới công ty</span>
      </v-card-title>
      <v-card-text>
        <v-form>
          <v-container>
      ...
      <v-card-actions>
        ...
        <v-btn color="grey darken-2" class="mr-2" dark @click="$emit('close')"
          >Huỷ</v-btn
        >
        <v-btn color="primary" class="mr-2" dark @click="addSupplier"
          >Lưu</v-btn
        >
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    addSupplier() {
      axios
        .post("https://localhost:44306/api/Supplier/InsertSupplier", this.data)
        .then(res => {
